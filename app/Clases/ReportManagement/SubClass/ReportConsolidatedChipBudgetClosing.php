...
class ReportConsolidatedChipBudgetClosing
{
        ...
        $querySQL = "DECLARE @year VARCHAR(6) = ?,
                ...
                , obligation_desagregado_before_modification AS (
                    ...
                    SELECT
                        ...
                        cd.budgetary,
                        CASE
                            WHEN ISNULL(ce.chip_code, 1) = 1 THEN ''
                            ELSE cd.nit
                        END AS nit,
                        CASE
                            WHEN ISNULL(ce.chip_code, 1) = 1 THEN ''
                            ELSE cd.nombre_tercero
                ...
                SELECT
                    ...
                    obligation_value,
                    paymentorder_value
                FROM consolidado ORDER BY category_code, category_name, financial_source_code, financial_source, dane_codes";
