...
class Menu extends Model
{
    ...
    {
        return $this->belongsToMany(Role::class, 'menu_rol', 'menu_id', 'roles_id');
    }
    ...
    public function getPadres($front)
    {
        if ($front) {
            ...
            if(session()->has('roles')){
                ...
                $roles_id = collect(session()->get('roles'))->pluck('id')->toArray();

                return $this->whereHas('roles', function ($query) use ($roles_id) {
                    $query->whereIn('roles_id', $roles_id)->orderby('menu_id');
                })->orderby('menu_id')
                    ->orderby('orden')
                    ->distinct()
                    ->get()
                ...
                return $this->whereHas('roles', function ($query) {
                    $query->where('roles_id', session()->get('rol_id'))->orderby('menu_id');
                })->orderby('menu_id')
                    ->orderby('orden')
                    ->get()
                    ->toArray();
