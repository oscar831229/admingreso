...
synchronizations = {
    ...
    viewsynchronizations : function(){

        tablsynchronizations= $('#tbl-synchronizations').DataTable();
        tablsynchronizations.destroy();

        $('#tbl-synchronizations thead th').each(function () {
            var title = $(this).text();
        ...
        tablsynchronizations = $('#tbl-synchronizations').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '/Admin/datatable-sisafi-synchronization',
                type: "POST",
                data: {
                    '_token'               : $('input[name=_token]').val(),
                    'type_synchronization' : $('#type_synchronization').val(),
                    'type_execution'       : $('#type_execution').val(),
        ...
        tablsynchronizations.columns().every(function () {
            var table = this;
        ...
        swal({
            ...
            if (value) {
                ...
                setTimeout(function(){
                    ...
                    $.ajax({
                        url: '/Admin/sisafi-synchronization',
                        async: true,
                        data: {
    ...
    init : function(){
        ...
        $('body').on('click', '#btn-refresh-liquidation', synchronizations.viewsynchronizations);
        $('body').on('click', '.btn-print-invoice'      , synchronizations.printInvoice);
        $('body').on('click', '.btn-reload-liquidation' , synchronizations.reloadLiquidation);
        $('body').on('click', '.btn-view-liquidation'   , synchronizations.viewLiquidationInvoice);
        $('body').on('click', '#btn-procesar-sync'      , synchronizations.syncAffiliates);

        // Obtener la fecha actual
        var fechaActual = new Date();

        var fecha_from = new Date();
        fecha_from.setDate(fechaActual.getDate() - 4);

        // Formatear las fechas según tus necesidades (opcional)
        var primerDiaFormateado = fecha_from.toISOString().split('T')[0];
