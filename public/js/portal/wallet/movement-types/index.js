...
step = {
  ...
  showListSteps : function(){
    ...
    $.each(step.steps, function(index, value){
      ...
      +'     <td>'+value.code+'</td>'
      +'     <td>'+value.name+'</td>'
      +'     <td>'+ step.getLableNaturaleza(value.nature_movement)+'</td>'
  ...
  getLableNaturaleza : function(state){

    switch (state) {
        case 'C':
            return '<label class="badge badge-primary"><i class="fa fa-plus-circle mr-2" aria-hidden="true"></i> Credito</label>';
            break;
        case 'D':
            return '<label class="badge badge-danger">Debito</label>';
            break;
  ...
  loadSteps : function(){
    ...
    $.ajax({
      url: '/wallet/details-movement-types',
      async: true,
      data: {},		
      beforeSend: function(objeto){
      },        
